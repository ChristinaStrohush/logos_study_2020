...
let checkNum: void = num > 0
    ? num % 2 == 0 ? console.log('Число парне') : console.log('Число непарне')
    : console.log('число 0')

// Task 3
function f1(a:number, b:number, ...arg:number[]): number{
...
// Task 4
let num2: any = +prompt('Вкажіть число для обрахунку квадратного кореня');
function getSqrt(number: any): void{
    while (number != number*1 || number <= 0) {
        if(number < 0) number = +prompt('Введдіть додатнє число');
        else if(number == '') number = +prompt('Будь ласка, введіть число!');
